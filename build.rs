...
fn main() {
    ...
    if !Command::new("make").args(&["-C", "v8_c_api/"]).status().expect("failed to compile v8_c_api").success() {
        panic!("failed to compile v8_c_api");
    }

    let output_dir = env::var("OUT_DIR").expect("Can not find out directory");

    if !Command::new("cp").args(&["v8_c_api/src/libv8.a", &output_dir]).status().expect("failed copy libv8.a to output directory").success() {
    ...
    println!("{}", format!("cargo:rustc-flags=-L{} -lv8 -lv8_monolith -lstdc++", output_dir))
}
